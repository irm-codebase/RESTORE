# --------------------------------------------------------------------------- #
# Filename: d_exp_simple.py
# Path: /d_exp_simple.py
...
# --------------------------------------------------------------------------- #
"""Simplified version of D-EXPANSE.

There are two NOTE items in this code. Keep them in mind when basing a project on this approach.
"""
import pandas as pd
import numpy as np
...
def cnf_model_constraints(mod: pyo.ConcreteModel, tech_df: pd.DataFrame, country_df: pd.DataFrame):
    ...
    for c in mod.DxpTechs:
        for d in mod.DxpDays:
            mod.init_p_cons.add(
                expr=sum(mod.p[c, y_0, d, h] for h in mod.DxpHours)
                == gen_y0[c] * 1000 * elec_supplied_ratio_y0[d] / (365 * k_ratio_y_d[y_0][d])
            )
    # All Imports/Exports
    for d in mod.DxpDays:
        mod.init_p_cons.add(
            expr=sum(mod.imp_p["Import", y_0, d, h] for h in mod.DxpHours)
            == imp_y0 * 1000 * elec_supplied_ratio_y0[d] / (365 * k_ratio_y_d[y_0][d])
        )
        mod.init_p_cons.add(
            expr=sum(mod.exp_p["Export", y_0, d, h] for h in mod.DxpHours)
    ...
    for y in mod.Years:
        for d in mod.DxpDays:
            for h in mod.DxpHours:
                prod_after_loss_expr = prod_after_ownuse_y(mod, y, d, h, own_use)
                prod_after_loss_expr += mod.line_p["Line", y, d, h] * (1 - own_use["Import"][y])
                mod.dem_cons.add(expr=prod_after_loss_expr == (demand_y_d_h[y][d][h]) * (1 + trans_loss[y]))
    ...
    lf_max = tech_df.loc["LF_max"].to_dict()  # NOTE: why are load factors above 1 for initial years?
    lf_min = tech_df.loc["LF_min"].to_dict()
    # for every flexible generator p(d,h) < p_nom * LF_max
    for c in mod.DxpTechs:
        for y in mod.Years:
            for d in mod.DxpDays:
                for h in mod.DxpHours:
    ...
    # NOTE: why is year zero omitted from all capacity constraints???
    ...
    for c in mod.DxpTechs:
        for y in mod.Years.ordered_data()[1:]:
            mod.p_potential_g_cons.add(
                expr=sum(
                    mod.p[c, y, d, h] / 1000 * 365 * k_ratio_y_d[y][d]
                    for h in mod.DxpHours
                    for d in mod.DxpDays
                )
